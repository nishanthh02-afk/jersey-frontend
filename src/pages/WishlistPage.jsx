    ...
    return (
        <div>
            ...
            <div className="px-4 md:px-12 py-6 md:py-8 border-b border-gray-200">
                <h1 className="font-bebas text-black text-4xl md:text-5xl tracking-wide">YOUR WISHLIST</h1>
                <p className="font-sans text-gray-500 text-sm mt-1">{wishlist.length} {wishlist.length === 1 ? 'item' : 'items'}</p>
            </div>

            {loading ? (
                <div className="min-h-screen flex items-center justify-center">
                    <p className="font-sans text-gray-500 text-sm tracking-widest">Loading your wishlist...</p>
                </div>
            ) : wishlist.length === 0 ? (
                <div className="min-h-[50vh] flex flex-col items-center justify-center gap-4">
                    <FiHeart size={48} className="text-gray-300" />
                    <p className="font-sans text-gray-500 text-sm">Your wishlist is empty</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                        {wishlist.map((item) => (
                            <div key={item.id} className="group">
                                <div
                                    className="bg-gray-100 h-48 md:h-80 relative overflow-hidden cursor-pointer"
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="absolute top-3 right-3 bg-white rounded-full p-1.5 shadow hover:bg-red-50"
                                    >
                                        <FiTrash2 size={16} className="text-gray-500 hover:text-red-500" />
                                    </button>
                                </div>

                                <div className="mt-3 cursor-pointer" onClick={() => navigate(`/products/${item.productId}`)}>
                                    <p className="font-sans text-black font-medium text-xs md:text-sm">{item.productName}</p>
                                    <p className="font-sans text-gray-500 text-xs md:text-sm mt-0.5">{item.league}</p>
                                    <p className="font-sans text-black font-semibold text-xs md:text-sm mt-1">₹{item.price?.toLocaleString()}</p>
                                </div>

                                <button
                                    onClick={() => navigate(`/products/${item.productId}`)}
                                    className="mt-3 w-full border border-black text-black font-sans text-xs tracking-widest py-2.5 hover:bg-black hover:text-white transition-all"
                                >
                                    SELECT SIZE
                                </button>
                            </div>
            ...
            <div className="bg-[#007CC6] w-full flex items-center justify-center px-6 md:px-20 py-16 mt-8">
                <div className="text-center">
                    <h2 className="font-bebas text-white text-4xl md:text-6xl tracking-widest">YOUR FAVOURITES.<br/>ALL IN ONE PLACE.</h2>
                    <p className="text-white font-sans text-base mt-4 opacity-80 tracking-wider">
                        Save your favourite jerseys and come back when you're ready to buy.
                    </p>
                </div>
            </div>
