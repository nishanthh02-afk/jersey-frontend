...
function ProductDetailPage() {
    ...
    return (
        <div>
            ...
            {/* Breadcrumb */}
            <div className="px-4 md:px-12 py-4 flex items-center gap-2 text-sm font-sans">
                ...
                <span className="text-gray-400">/</span>
                <span className="text-gray-500 truncate max-w-[120px] md:max-w-none">{product.name}</span>
            </div>

            {/* Product Detail */}
            <div className="px-4 md:px-12 py-6 md:py-8 flex flex-col md:flex-row gap-8 md:gap-16">

                {/* Left — Image Gallery */}
                <div className="flex gap-3 md:gap-4 w-full md:w-3/5">
                    {/* Thumbnails */}
                    <div className="flex flex-col gap-2 md:gap-3 w-14 md:w-20 flex-shrink-0">
                        {images.length > 0 ? images.map((img, index) => (
                            <div
                                key={index}
                                onClick={() => setSelectedImage(index)}
                                className={`w-14 h-14 md:w-20 md:h-20 bg-gray-100 cursor-pointer overflow-hidden border-2 ${selectedImage === index ? 'border-black' : 'border-transparent'}`}
                            >
                                <img src={img.imageUrl} alt={product.name} className="w-full h-full object-cover" />
                            </div>
                        )) : (
                            <div className="w-14 h-14 md:w-20 md:h-20 bg-gray-100 border-2 border-black" />
                        )}
                    </div>

                    {/* Main Image */}
                    <div className="flex-1 bg-gray-100 relative overflow-hidden" style={{minHeight: '300px'}}>
                        {images.length > 0 ? (
                            <img
                        ...
                        {images.length > 1 && (
                            <>
                                <button
                                    ...
                                    className="absolute left-2 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 md:p-2 shadow hover:bg-gray-100"
                                >←</button>
                                <button
                                    onClick={() => setSelectedImage(prev => prev === images.length - 1 ? 0 : prev + 1)}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 md:p-2 shadow hover:bg-gray-100"
                                >→</button>
                            </>
                        )}
                    </div>
                ...
                {/* Right — Product Info */}
                <div className="w-full md:w-2/5 flex flex-col pt-0 md:pt-4">
                    <p className="font-sans text-gray-500 text-sm tracking-widest uppercase">{product.league}</p>
                    <h1 className="font-sans text-black text-2xl md:text-3xl font-bold mt-2 leading-tight uppercase">{product.name}</h1>
                    <p className="font-sans text-gray-500 text-sm mt-1">{product.brand} · {product.team}</p>

                    <p className="font-sans text-black text-2xl md:text-3xl font-semibold mt-6">₹{product.price?.toLocaleString()}</p>
                            ...
                            <div className="grid grid-cols-3 gap-2 md:gap-3">
                                {variants.map((variant) => (
                                    <button
                                        ...
                                        className={`border py-2.5 md:py-3 text-sm font-sans font-medium transition-all ${
                    ...
                    <button
                        ...
                        className="mt-6 md:mt-8 bg-black text-white font-sans font-medium tracking-widest text-sm py-4 w-full hover:bg-gray-800 disabled:opacity-50"
                    >
            ...
            {/* Size Guide Modal */}
            {showSizeGuide && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4" onClick={() => setShowSizeGuide(false)}>
                    <div className="bg-white p-6 md:p-8 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Green banner */}
            <div className="bg-[#007CC6] w-full flex items-center justify-center px-6 md:px-20 py-16 mt-8 md:mt-16">
                <div className="text-center">
                    <h2 className="font-bebas text-white text-4xl md:text-6xl tracking-widest">PLAYER QUALITY.<br/>YOUR SIZE.</h2>
